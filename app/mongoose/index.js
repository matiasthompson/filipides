const mongoose = require("mongoose"),
  { setCategory } = require("../utils/logEnv"),
...
const initMongoose = () => {
  mongoose.connect(uri, options).then(context => {
    logger.info(
      `Connection to MongoDB Success! DB: (${process.env.MONGO_DB}), HOST: (${
        process.env.MONGO_HOST_0
      }:${process.env.MONGO_DEFAULT_PORT})`
    );
  });
};

events.forEach((event, err) => {
  mongoose.connection.on(event, function(a) {
...
const uri =
  "mongodb+srv://bi:despegar01@filipides-kr5h3.mongodb.net/test?authSource=admin&readPreference=primary&ssl=true";
