...
// Create HTTPS server.
const { initApollo } = require("./app/apollo"),
...
server.listen(app.settings.port, () =>
  logger.info(
    `ð Express server ready at http://${process.env.DOMAIN_BACK}/${
      process.env.ENDPOINT_PATH
    }/${process.env.APP_PATH}  with PID: ${process.pid}`
  )
);
...
function onError(error) {
  ...
  const bind =
    typeof app.settings.port === "string"
